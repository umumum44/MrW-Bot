...
module.exports.run = async (bot, message, args) => {
	...
	if(message.content.substr(8).length < 5) {
		choice = message.guild.roles.find(role => role.name.toLowerCase() === args[0].toLowerCase());
		...
		choice = message.guild.roles.find(role => role.name.toLowerCase().startsWith(args[0].toLowerCase()) || args[0].includes(`${role.id}`));
